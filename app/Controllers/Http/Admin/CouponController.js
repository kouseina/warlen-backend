...
class CouponController {
	...
	async store({ request, response, transform }) {
		...
		try {
			...
			await trx.rollback()
			return response.status(400).send({ message: 'The coupon could not be created at this time!' })
		}
	}
	...
	async update({ params: { id }, request, response, transform }) {
		...
		try {
			...
			return response.status(400).send({
				message: 'This coupon could not be updated at this time!'
			})
		}
	}
	...
	async destroy({ params: { id }, request, response }) {
		...
		try {
			...
			await trx.rollback()
			return response.status(400).send({
				message: 'This coupon could not be deleted at this time!'
			})
		}
	}
}
