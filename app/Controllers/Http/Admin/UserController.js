...
class UserController {
	...
	async store({ request, response, transform }) {
		try {
			...
			return response.status(400).send({ message: 'This user could not be created at this time!' })
		}
	}
	...
	async destroy({ params: { id }, request, response }) {
		...
		try {
			...
			response.status(500).send({ message: 'It was not possible to delete this user!' })
		}
	}
}
