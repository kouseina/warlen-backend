...
class ProductController {
	...
	async store({ request, response, transform }) {
		try {
			...
			response.status(400).send({ message: 'The product could not be created at this time!' })
		}
	}
	...
	async update({ params: { id }, request, response, transform }) {
		...
		try {
			...
			return response.status(400).send({ message: 'This product could not be updated!' })
		}
	}
	...
	async destroy({ params: { id }, request, response }) {
		...
		try {
			...
			return response.status(500).send({ message: 'This product could not be deleted!' })
		}
	}
}
